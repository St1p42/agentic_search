...
def test_chunk_ranker_prefers_best_rewrite_plus_support_bonus() -> None:
    ...
    assert len(output.ranked_chunks) == 2
    assert output.ranked_chunks[0].source_id == "jina:https://alpha.example/about"
    assert output.ranked_chunks[0].score.best_rewrite_score > 0.0
    assert output.ranked_chunks[0].score.query_variant_coverage_count >= 1
    assert output.ranked_chunks[0].score.query_variant_coverage_score > 0.0
    assert output.ranked_chunks[0].score.max_query_span_score > 0.0
    assert output.ranked_chunks[0].score.anchor_coverage_score > 0.0
    assert output.ranked_chunks[0].score.final_score > output.ranked_chunks[1].score.final_score
